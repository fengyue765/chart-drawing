...
def parse_log_file(file_path, epoch_interval=1):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            match = re.search(r'Epoch (\d+):.*RMSE_F=([\d.]+) meV / A', line)
            if match:
...
def plot(settings):
    ...
    plt.ylabel("Force loss (meV/Å)", fontsize=24)
    ...
    # 保存和显示
    plt.savefig(f"{output_dir}/{molecule}_force_loss.png", dpi=300, bbox_inches='tight')